...
public class Contract {
    ...
    public void createUser(String wallet, String name, int balance, Roles Role, String homeAddress, String jobPlace, String identifier) {
    ...
    public void SendPostal(
        ...
        if (postalClass == PostalClass.First) {
            timer = 5;
            priceOfDelivery = 0.5F;
        } else if (postalClass == PostalClass.Second) {
            timer = 10;
        ...
        User whoSending = users.get(startAddress);
        if (whoSending.balance >= (int) totalValue) {
            whoSending.balance -= (int) totalValue;
            newPostal.status = Status.Confirmed;
        } else {
            newPostal.status = Status.Declined;
        }
        dayCounter++;
    ...
    public void ReceivePostal(String trackingNumber) {
        ...
        if (Objects.equals(postal.receiver, call.getCaller())) postal.status = Status.Received;
    ...
    public void DeclinePostal(String trackingNumber) {
        Postal postal = postals.get(trackingNumber);
        if (Objects.equals(postal.receiver, call.getCaller())) postal.status = Status.Declined;
    ...
    /// ////////////////////////////переводы///////////////////////////////
    @ContractAction
    public void createMoneyTransfer(String _sender, String receiver, int value, int timeLeft) {
        Transfer transfer = new Transfer(_sender, receiver, value, timeLeft); // timeleft это сколько секунд перевод живет
